<?php
	...
	if(isset($_POST['add'])){
		...
		$genre_id = mysqli_real_escape_string($conn, $genre);

		// add image
		if(isset($_FILES['image']) && $_FILES['image']['name'] != ""){
			$image = $_FILES['image']['name'];
            $uploadDirectory = $_SERVER['DOCUMENT_ROOT'] . DIRECTORY_SEPARATOR  ."bootstrap" . DIRECTORY_SEPARATOR . "img" . DIRECTORY_SEPARATOR . $image;
			move_uploaded_file($_FILES['image']['tmp_name'], $uploadDirectory);
		}

		$query = "INSERT INTO books (`book_isbn`, `book_title`, `book_author`, `book_image`, `book_descr`, `book_price`, `genre_id`) VALUES ('" . $isbn . "', '" . $title . "', '" . $author . "', '" . $image . "', '" . $descr . "', '" . $price . "', '" . $genre_id . "')";
		$result = mysqli_query($conn, $query);
		if($result){
			$_SESSION['book_success'] = "New Book has been added successfully";
			header("Location:admin_admin_book.php");
		}else {
						...
						<form method="post" action="admin_add_book.php" enctype="multipart/form-data">
								...
								<div class="mb-3">
									...
									<select class="form-select rounded-0"  name="genre" required>
										...
										<?php
										$psql = mysqli_query($conn, "SELECT * FROM `genre` order by genre_name asc");
